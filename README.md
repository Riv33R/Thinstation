...
| :--- | :--- | :--- |
| `SESSION_0_TITLE` / `SERVER` | `VDI` | Идентификатор сессии (без пробелов для корректной передачи параметров) |
...
| `HORIZON_DEFAULTBROKER` | `https://vdi.dnestrschool1.online` | Жестко заданный брокер по умолчанию (сервер предзаполнен всегда) |
| `HORIZON_AUTOCONNECT` | `TRUE` | Автоматический старт подключения к серверу при загрузке |
...
| `HORIZON_SSLVERIFYMODE` | `3` | Режим проверки SSL (не блокирует вход при сброшенных часах или нехватке CA) |
...
| `WIRELESS_ESSID` / `WIRELESS_WPAKEY` | *(опция)* | Предварительно заданные SSID и пароль Wi-Fi для автоподключения |

> [!NOTE]
> В образ включена служба **NetworkManager** с апплетом **nm-applet** в системном трее. Если Wi-Fi сеть не преднастроена в конфиге, пользователь может кликнуть на иконку сети в правом нижнем углу панели, выбрать нужный Wi-Fi SSID и ввести пароль.
