...
├── CHANGELOG.md                    # Детальный журнал изменений, анализ и устранение ошибок
...
## 📋 История изменений и архитектурных решений

Подробная техническая документация по всем этапам разработки, исправлению сбоев загрузки ядра, матрице драйверов и оптимизациям доступна в файле [**`CHANGELOG.md`**](CHANGELOG.md).

### Краткая сводка ключевых решений:
* **Устранение паники ядра `0x00007f00`:** Отключен механизм `fastboot` (`param fastboot false`), вырезавший библиотеки и модули из `initrd` в отдельный `lib.squash`. Система теперь полностью упакована в целостный XZ-сжатый `initrd` со всеми разделяемыми библиотеками и обновлением `ldconfig`.
* **Универсальная поддержка оборудования (All Modules):** Включен полный пакет модулей ядра (`--allmodules`), гарантирующий работу на любых сетевых адаптерах (Realtek, Intel, Broadcom, Atheros, USB-Ethernet) и контроллерах накопителей (NVMe, AHCI SATA, USB 3.0 UAS).
* **Графика и дисплейный менеджер:** Добавлены DRM-драйверы ядра `i915`, `intel_gtt`, `amdgpu`, `radeon`, `nouveau`, `simpledrm`, резервные драйверы X.Org `vesa` и `fbdev`, а также прошивки GPU (Intel Iris 640 DMC/GuC/HuC) и права доступа `1777` к каталогам `lightdm`.
* **Сетевая оптимизация:** Отключен блокирующий сервис `NetworkManager-wait-online` (старт за 3 секунды без задержки в 90 с), включен внутренний DHCP, настроен системный апплет `nm-applet` в трее для выбора Wi-Fi сетей и разблокировка `rfkill`.
* **Omnissa Horizon Client:** Настроена принудительная адресация `https://vdi.dnestrschool1.online`, режим SSL 3, развертывание на все мониторы (`Extended Desktop`) и безопасная обертка CLI.
